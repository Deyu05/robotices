...
class ExplorationServer(Node):
    ...
    def lidar_callback(self, scan_data: LaserScan): 
        ...
        f_left = scan_data.ranges[20:31]
        f_right = scan_data.ranges[-30:-20] 
        right_side_deg = scan_data.ranges[270:300] 
        left_side_deg = scan_data.ranges[60:90] 
    ...
    async def server_execution_callback(self, goal_handle):
        ...
        while not(goal_handle.is_cancel_requested):
            ...
            if self.front_dist[0] > stop_dist + 0.3 and self.front_dist[1] > stop_dist + 0.3 and self.front_dist[2] > stop_dist + 0.3:
            ...
            # turn right into corner
            if self.initial_turn:
                if self.front_dist[0] < stop_dist + 0.3 or self.front_dist[0] == float("nan"):
                ...
                # # turning robot left when object in front or the (front) left sensor is detecting an object close
                # elif self.front_dist[0] <= stop_dist + 0.05 or self.front_dist[1] < self.front_dist[0] / 2.5:
                ...
                elif self.front_dist[3] > 1.5 and self.front_dist[1] > 1.5 and self.front_dist[5] > 1.5:
                    vel_cmd.linear.x = 0.02
                    vel_cmd.angular.z = -turn_vel
